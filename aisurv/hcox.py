...
class _(cox._):
    ...
    def coefficient(self,index=None):
        ...
        if self.split is False:
            ...
            omega_alpha = self._intermediate['omega_alpha'] = self._projection(infom=infom,hd=hd_alpha) # infom_inv.dot(hd_alpha)
    ...
    def hazard_ratio(self,index=None):
        ...
        if self.split is False:
            ...
            omega_hr = self._intermediate['omega_hr'] = self._projection(infom=infom,hd=hd_hr)
            hr = hr_lasso + omega_hr.T.dot(score)

            # - variance
            hr_influence = self._hazard_ratio_influence(
                beta=beta_lasso,index=index,omega=omega_hr)
    ...
    def survival_probability(self,time,x,cross=True):
        ...
        if self.split is False:
            ...
            num_time = len(time)
            if cross is False:
                hd_sp = - (hd_bch+x.T*Lamt)*sp_lasso*exb  
                omega_sp = self._projection(infom=infom,hd=hd_sp) # np.linalg.inv(infom).dot(hd_sp) # 
                sp = sp_lasso + omega_sp.T.dot(score)
            else:
                num_x = x.shape[0]
                sp = np.zeros(shape=(num_time,num_x))
                omega_sp = np.zeros((self.p,num_x,num_time))
                for itime in range(len(time)):
                    hd_sp_ = - (hd_bch[:,[itime]]+x.T*Lamt[itime])*sp_lasso[itime,:]*exb  
                    omega_sp[:,:,itime] = self._projection(infom=infom,hd=hd_sp_)
                    sp[itime,:] = sp_lasso[itime,:] + omega_sp[:,:,itime].T.dot(score)
            self._intermediate['omega_sp'] = omega_sp

            # - variance
            sp_influence = self._survival_probability_influence(
                beta=beta_lasso,time=time,x=x,cross=cross,omega=omega_sp)
    ...
    def _lasso(self,rowidx=None,colidx=None):
        ...
        tunpara_scale = np.exp(np.linspace(
            start=np.log(50),stop=np.log(1/50),num=self.tunpara_num))
        tunpara = np.sqrt(np.log(p)/n)*tunpara_scale
    ...
    def _projection(self,infom,hd):
        ...
        # prepare candidate set of tuning parameters
        tunpara_scale = np.exp(np.linspace(
            start=np.log(5),stop=np.log(1/5),num=self.tunpara_num))
        tunpara = np.sqrt(np.log(self.p)/self.n)*tunpara_scale
        # tunpara = np.sqrt(1/self.n)*self.tunpara_scale
...
class kmsp(_,cox.kmsp): 
    ...
    def coefficient(self,index=None):
        ...
        if self.split is False:
            ...
            omega_alpha = self.model_ori._intermediate['omega_alpha']
            alpha_ori_influence = self.model_ori._coefficient_influence(
                index=index,beta=beta_lasso,omega=omega_alpha)
    ...
    def hazard_ratio(self,index=None):
        ...
        if self.split is False:
            ...
            beta_lasso = self.model_ori._intermediate['beta_lasso']
            omega_hr = self.model_ori._intermediate['omega_hr']
            hr_ori_influence = self.model_ori._hazard_ratio_influence(
                index=index,beta=beta_lasso,omega=omega_hr)
    ...
    def survival_probability(self,time,x,cross=True):
        ...
        if self.split is False:
            ...
            beta_lasso = self.model_ori._intermediate['beta_lasso']
            omega_sp = self.model_ori._intermediate['omega_sp']
            sp_ori_influence = self.model_ori._survival_probability_influence(
                time=time,x=x,beta=beta_lasso,cross=cross,omega=omega_sp)
