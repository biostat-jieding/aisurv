...
class _(km._):
    ...
    ## estimation and inference on baseline cumulative hazard function at given time
    def average_treatment_effect(self,time): # ,type="survival_probability"
        
        # preparations
        if 'beta' not in self._intermediate:
            self._intermediate['beta'] = self._MPLE()
        beta = self._intermediate['beta']
        ate = self._Breslow(time=time,beta=beta)

        # influence function and the associated variance 
        ate_influence = self._baseline_cumulative_hazard_influence(
            time=time,beta=beta)
        ate_avar = (ate_influence**2).sum(axis=0)/self.n
        ate_se = np.sqrt(ate_avar/self.n)

        # store fitted values
        self.result['baseline_cumulative_hazard'] = {'estimate':ate}
        self.result['baseline_cumulative_hazard']['se'] = ate_se

    ## influence of baseline cumulative hazard function
    def _average_treatment_effect_influence(self,time,beta):
    ...
    def _hazard_ratio_influence(self,beta,index=None,omega=None):
        ...
        if omega is None:
            ...
            hd_alpha = np.zeros(shape=(self.p,len(index)))
            hd_alpha[index,:] = np.eye(len(index))
            hd_hr = hd_alpha.dot(np.diag(hr))
            omega = infom_inv.dot(hd_hr)
    ...
    def _survival_probability_influence(self,time,x,beta,cross=True,omega=None):
        ...
        if cross is False:
            if omega is None:
                ...
                omega = infom_inv.dot(hd_sp)
            influence = -Lamt_influence_1*sp*exb + influence_score.dot(omega)
        else: 
            influence = np.zeros(shape=(self.n,len(time),x.shape[0]))
            for itime in range(len(time)):
                if omega is None:
                    hd_sp_ = - (hd_bch[:,[itime]]+x.T*Lamt[itime])*sp[itime,:]*exb  
                    omega_ = infom_inv.dot(hd_sp_)
                else:
                    omega_ = omega[:,:,itime]
                influence[:,itime,:] = - Lamt_influence_1[:,[itime]]*sp[itime,:]*exb + influence_score.dot(omega_)
